...
for k=5%1:num_dataset
    ...
    for i=1:num_dp  %ISL10: 1422%1418%1571%1495%1397%1433%1445%1502  %ISL12:  7%1383% %ISL11:728%1402% %ISL1:7%176%173%4%12%      
%         if auto_kept_not_manual(k,i)%manual_kept_notauto(k,i)
        f1 = figure(1);
        clf(f1)
        t = t_center{k};             % time
        ...
        if numel(t)>5
            ...
            if plot_interm
                ...
                hTitle1=title([file_in{k}(1:end-4) ' - DP' num2str(i)]);
            end
...
%                             keyboard
            ...
            if isempty(locs)
                warning(['no max found for file' file_in{k}(1:end-4) ' - DP ' num2str(i)])
            elseif numel(t)>5
...
%                         points_to_keep_EM=find(abs(relative_residuals_Krafla_EM)<rel_res_thres);
                ...
                warning(['not enough data left for polyfit' file_in{k}(1:end-4) ' - DP ' num2str(i)])
            end
            
            if sum(Mapp_cell{k}(i,:))==0
                warning(['no IP data for' file_in{k}(1:end-4) ' - DP ' num2str(i)])
            else
                clf;
...
%             if ismember(i,[1675,1417,1422,1413,1673,1408,1383,1669,1656,1410,1402])
                ...
                loglog(hAx1,t_center{k}, abs(Mapp_cell{k}(i,:)),'or','MarkerFaceColor','r','MarkerSize',3)
                hold(hAx1,'on')
                loglog(hAx1,t_center{k}, Mapp_cell{k}(i,:),'ob','MarkerFaceColor','b','MarkerSize',3)

                xlim([1e0,1e4])
                ylim([1e-1,1e4])
                plot(hAx1,my_time,exp(logm),'--b','LineWidth',0.5)
                hXLabel1=xlabel('Time (msec)');
                hYLabel1=ylabel('Polarizability \eta (mV/V)');

                plot(hAx1,remaining_gates{k,i},remaining_m{k,i},'o','Color','g','MarkerFaceColor',[0 1 0],'MarkerSize',6);

                if ~isempty(gof_se{k, i})
                    if numel(xdata0_excl_EM{k,i})>6
                        plot(hAx1,remaining_gates{k,i},remaining_m{k,i},'x','Color','k','MarkerSize',8);
                        plot(hAx1,xdata0_excl_EM{k,i},ydata0_excl_EM{k,i},'o','Color','k','MarkerFaceColor','k','MarkerSize',6);
                        plot(hAx1,xdata0_excl_EM{k,i},se_fit_mapp_2{k,i},'-','Color','k','LineWidth',2);
                    ...
                    elseif gof_se{k, i}.rsquare>0.9                       
                        plot(hAx1,remaining_gates{k,i},se_fit_mapp{k,i},'-','Color','k','LineWidth',2)
                ...
                hTitle1=title([file_in{k}(1:end-4) ' - DP' num2str(i)]);
                yyaxis(hAx1,'right')
                plot(hAx1,my_time,clog)
                hYLabel2=ylabel('Curvature');
                hold(hAx1,'on')

                grid on
                text(all_locs+.02,val_clog_peak,num2str((1:numel(val_clog_peak))'))
                plot(hAx1,my_time,clog,all_locs,val_clog_peak,'r*')
...
%                 name_out=['png\Krafla_SEfit_EM_deg4_normr1\' file_in{k}(1:end-4) '_DP' num2str(i) '_SEfit_EM_deg4_v3.png'];
%                 name_out=['png\Krafla_SEfit_EM_deg5_v2\' file_in{k}(1:end-4) '_DP' num2str(i) '_SEfit_EM_deg5.png'];
                name_out=['pdf\ISL10\' file_in{k}(1:end-4) '_DP' num2str(i) '_SEfit_EM_deg5.pdf'];

                exportgraphics(gcf,name_out,'Resolution',300)
%                 keyboard
            end
        else
            warning(['Not enough gates for file' file_in{k}(1:end-4) ' - DP ' num2str(i)])
        end
%         end
    end
...
for k=1:num_dataset
    if ~isempty(my_voltage{k})
        plot(hAx2,Res_save{k}(time_curvature_max_all{k}(:,2)==1),time_curvature_max_all{k}(time_curvature_max_all{k}(:,2)==1,1),'ok','MarkerFaceColor',col(k,:))
        hold(hAx2,'on')
    end
end
set(hAx2, 'XScale', 'log')
...
%% save mat file
name_final='mat\Krafla_SEfit_log_curvatureMax_Min_End_deg5_v2.mat';
